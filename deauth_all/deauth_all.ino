// this is from the GitHub ESP deauth all thingy.
...
Adafruit_SSD1306 Serial(OLED_RESET);
...
int channel = 1; // STARTING CHANNEL
...
void send_deauth(AccessPoint access_point)
{
  ...
  WiFi.channel(access_point.channel);
  delay(1);
  
  // SEND DEAUTH PACKET
  WiFi.send_pkt_freedom(access_point.deauthPacket, 26, 0);
}
...
// ok what? google says ICACHE_FLASH_ATTR is something about runnign from IRAM or something - v205 
...
void  promisc_cb(uint8_t *buf, uint16_t len)
{
  ...
  { 
    ...
    if (sniffer->buf[0] == 0x80)
    {
      ...
      if (add_access_point(byte_arr, channel, local_essid, sniffer->rx_ctrl.rssi))
      {
        Serial.print("Beacon -> ");
...
void scan()
{
  ...
  //No idea what these do - V205 so I commented them
  /*
  WiFi.promiscuous_enable(0);
  WiFi.set_promiscuous_rx_cb(promisc_cb);
  WiFi.promiscuous_enable(1);
*/
...
void setup() {
  ...
  Serial.begin();
  /*
  Serial.begin(SSD1306_SWITCHCAPVCC, 0x3C); 
  Serial.setCursor(0,0);
  ...
  */
  ...
  //Ok what on earth does optmode do - v205 commented
